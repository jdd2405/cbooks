...
function login($email, $password, $mysqli) {
    ...
    if ($result = $mysqli->query("SELECT id_cb_user, email, password, salt 
        FROM cb_users
        WHERE email = '".$email."'
        LIMIT 1")) {
        ...
        if ($result->num_rows == 1) {
            ...
            /*if (checkbrute($row['user_id'], $mysqli) == true) {
                // Konto ist blockiert 
                ...
                if ($row['password'] == $password) {
                    ...
                    $user_id = preg_replace("/[^0-9]+/", "", $row['id_cb_user']);
