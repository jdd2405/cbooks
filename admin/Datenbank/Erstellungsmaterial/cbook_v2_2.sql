-- ----------- P I M P _ T H E _ S K R I P T -----------
--                                                    --
--    Rename DBs: 'mydb' > 'usr_p180487_2'            --
--    Constraints FK for books and personal_books     --
--             rEMOVE Comments                                       --
-- 2DO: Neu Login Attempts Table. VGl Mail von Jones  --
--                                                    --
-- ----------------------- E N D -----------------------



SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

CREATE SCHEMA IF NOT EXISTS `usr_p180487_2` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `usr_p180487_2` ;

-- -----------------------------------------------------
-- Table `usr_p180487_2`.`languages`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`languages` (
  `id_language` INT NOT NULL AUTO_INCREMENT,
  `lng_name` VARCHAR(45) NULL,
  PRIMARY KEY (`id_language`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`books`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`books` (
  `id_isbn` VARCHAR(17) NOT NULL,
  `title` VARCHAR(250) NULL,
  `subtitle` VARCHAR(250) NULL,
  `blurb` TEXT NULL,
  `language` INT NULL,
  `image_link` VARCHAR(24) NULL,
  `reg_date` TIMESTAMP NULL,
  `publisher` VARCHAR(45) NULL,
  `publication_date` DATE NULL,
  UNIQUE INDEX `id_isbn_UNIQUE` (`id_isbn` ASC),
  PRIMARY KEY (`id_isbn`)
  )
  
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`cb_users`  pword not null
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`cb_users` (
  `id_cb_user` INT NOT NULL AUTO_INCREMENT,
  `title` SET('h','f','b') NULL DEFAULT 'b',
  `username` VARCHAR(100) NULL,
  `pword` CHAR(128) NULL,
  `salt` CHAR(128) NULL,
  `birth_date` DATE NULL,
  `first_name` VARCHAR(100) NULL,
  `family_name` VARCHAR(100) NULL,
  `street` VARCHAR(100) NULL,
  `street_num` VARCHAR(4) NULL,
  `zip` VARCHAR(10) NULL,
  `city` VARCHAR(45) NULL,
  `country` VARCHAR(45) NULL,
  `access_right` SET('u','s','l') NULL DEFAULT 'l',
  `reg_date` TIMESTAMP NULL,
  `last_activity` DATETIME NULL,
  `language` INT NULL,
  PRIMARY KEY (`id_cb_user`),
  UNIQUE INDEX `username_UNIQUE` (`username` ASC),
  INDEX `fk_cb_users_languages1_idx` (`language` ASC),
  CONSTRAINT `fk_cb_users_languages1`
    FOREIGN KEY (`language`)
    REFERENCES `usr_p180487_2`.`languages` (`id_language`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`personal_books`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`personal_books` (
  `id_personal_book` INT NOT NULL AUTO_INCREMENT,
  `isbn` VARCHAR(17) NULL,
  `description` TEXT NULL,
  `availability` SET('a','l','i') NULL DEFAULT 'a',
  `owner_id_user` INT NULL,
  `reg_date` TIMESTAMP NULL,
  `last_activity` DATETIME NULL,
  `run` VARCHAR(45) NULL,
  `volume` VARCHAR(45) NULL,
  PRIMARY KEY (`id_personal_book`),
  INDEX `fk_objects_user1_idx` (`owner_id_user` ASC),
  INDEX `fk_personal_books_books1_idx` (`isbn` ASC),
  CONSTRAINT `fk_objects_user10`
    FOREIGN KEY (`owner_id_user`)
    REFERENCES `usr_p180487_2`.`cb_users` (`id_cb_user`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`lending_relations`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`lending_relations` (
  `id_lending_relation` INT NOT NULL AUTO_INCREMENT,
  `requestDate` DATE NULL,
  `authorizationDate` DATE NULL,
  `returnDate` DATE NULL,
  `state` SET('r','a','c','l') NULL DEFAULT 'r',
  `lender_id_user` INT NULL,
  `item_id_personal_book` INT NULL,
  `addressString` VARCHAR(500) NULL,
  PRIMARY KEY (`id_lending_relation`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`authors`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`authors` (
  `id_author` INT NOT NULL AUTO_INCREMENT,
  `aut_name` VARCHAR(100) NULL,
  PRIMARY KEY (`id_author`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`categorys`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`categorys` (
  `id_categorys` INT NOT NULL,
  `cat_name` VARCHAR(45) NULL,
  PRIMARY KEY (`id_categorys`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`books_has_authors`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`books_has_authors` (
  `books_id_isbn` VARCHAR(17) NOT NULL,
  `authors_id_author` INT NOT NULL,
  PRIMARY KEY (`books_id_isbn`, `authors_id_author`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `usr_p180487_2`.`books_has_categorys`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usr_p180487_2`.`books_has_categorys` (
  `books_id_isbn` VARCHAR(17) NOT NULL,
  `categorys_id_categorys` INT NOT NULL,
  PRIMARY KEY (`books_id_isbn`, `categorys_id_categorys`))
ENGINE = InnoDB;

-- -----------------------------------------------------
-- Add Foreign Key to table `books`
-- -----------------------------------------------------
ALTER TABLE `books`
  ADD INDEX `fk_books_languages1_idx` (`language` ASC),
  ADD CONSTRAINT `fk_books_languages1`
    FOREIGN KEY (`language`)
    REFERENCES `usr_p180487_2`.`languages` (`id_language`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Add Foreign Key to table `personal_books`
-- -----------------------------------------------------
ALTER TABLE `personal_books`
  ADD CONSTRAINT `fk_personal_books_books1`
    FOREIGN KEY (`isbn`)
    REFERENCES `usr_p180487_2`.`books` (`id_isbn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
ENGINE = InnoDB;

-- -----------------------------------------------------
-- Add Foreign Key to table `lending_relations`
-- -----------------------------------------------------
ALTER TABLE `lending_relations`
  ADD INDEX `fk_relation_user2_idx` (`lender_id_user` ASC),
  ADD INDEX `fk_lending_relations_personal_books1_idx` (`item_id_personal_book` ASC),
  ADD CONSTRAINT `fk_relation_user2`
    FOREIGN KEY (`lender_id_user`)
    REFERENCES `usr_p180487_2`.`cb_users` (`id_cb_user`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_lending_relations_personal_books1`
    FOREIGN KEY (`item_id_personal_book`)
    REFERENCES `usr_p180487_2`.`personal_books` (`id_personal_book`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
ENGINE = InnoDB;

-- -----------------------------------------------------
-- Add Foreign Key to table `books_has_authors`
-- -----------------------------------------------------
ALTER TABLE `books_has_authors`  
  ADD INDEX `fk_books_has_authors_authors1_idx` (`authors_id_author` ASC),
  ADD INDEX `fk_books_has_authors_books1_idx` (`books_id_isbn` ASC),
  ADD CONSTRAINT `fk_books_has_authors_books1`
    FOREIGN KEY (`books_id_isbn`)
    REFERENCES `usr_p180487_2`.`books` (`id_isbn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_books_has_authors_authors1`
    FOREIGN KEY (`authors_id_author`)
    REFERENCES `usr_p180487_2`.`authors` (`id_author`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
ENGINE = InnoDB;
	
-- -----------------------------------------------------
-- Add Foreign Key to table `books_has_categorys`
-- -----------------------------------------------------
ALTER TABLE `books_has_categorys`
  ADD INDEX `fk_books_has_categorys_categorys1_idx` (`categorys_id_categorys` ASC),
  ADD INDEX `fk_books_has_categorys_books1_idx` (`books_id_isbn` ASC),
  ADD CONSTRAINT `fk_books_has_categorys_books1`
    FOREIGN KEY (`books_id_isbn`)
    REFERENCES `usr_p180487_2`.`books` (`id_isbn`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_books_has_categorys_categorys1`
    FOREIGN KEY (`categorys_id_categorys`)
    REFERENCES `usr_p180487_2`.`categorys` (`id_categorys`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
